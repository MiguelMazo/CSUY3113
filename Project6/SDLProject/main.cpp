...
void ProcessInput() {
    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_KEYDOWN:
            ...
            case SDLK_SPACE:
                ...
                currentScene->state.player->shootProjectile(currentScene->state.player->projectileType, 1);
    ...
    if (keys[SDL_SCANCODE_LEFT] & keys[SDL_SCANCODE_UP]) {
        ...
        currentScene->state.player->direction = UPLEFT;
    }
    else if (keys[SDL_SCANCODE_LEFT] & keys[SDL_SCANCODE_DOWN]) {
        currentScene->state.player->movement.x += -1.0f;
        currentScene->state.player->movement.y += -1.0f;
        currentScene->state.player->animIndices = currentScene->state.player->animLeft;
        currentScene->state.player->direction = DOWNLEFT;
    }
    else if (keys[SDL_SCANCODE_RIGHT] & keys[SDL_SCANCODE_DOWN]) {
        currentScene->state.player->movement.x += 1.0f;
        currentScene->state.player->movement.y += -1.0f;
        currentScene->state.player->animIndices = currentScene->state.player->animRight;
        currentScene->state.player->direction = DOWNRIGHT;
    }
    else if (keys[SDL_SCANCODE_RIGHT] & keys[SDL_SCANCODE_UP]) {
        currentScene->state.player->movement.x += 1.0f;
        currentScene->state.player->movement.y += 1.0f;
        currentScene->state.player->animIndices = currentScene->state.player->animRight;
        currentScene->state.player->direction = UPRIGHT;
    }
...
void Render() {
    ...
    if (status == -1) {
        Util::DrawText(&program, fontTextureID, "RATRACE", 0.4f, 0.01f, glm::vec3(1, -2, 0));
        Util::DrawText(&program, fontTextureID, "PRESS ENTER TO START", 0.4f, 0.01f, glm::vec3(1, -4, 0));
    }
    else if (status == 1) {
        Util::DrawText(&program, fontTextureID, "YOU LOSE", 0.4f, 0.01f, glm::vec3(currentScene->state.player->position.x, currentScene->state.player->position.y, 0));
    }
    else if (status == 2) {
        Util::DrawText(&program, fontTextureID, "YOU WIN", 0.4f, 0.01f, glm::vec3(currentScene->state.player->position.x, currentScene->state.player->position.y, 0));
    }
